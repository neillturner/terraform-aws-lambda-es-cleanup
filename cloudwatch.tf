...
resource "aws_cloudwatch_event_target" "es_cleanup" {
  ...
  arn       = "${aws_lambda_function.es_cleanup[0].arn}"
}
...
resource "aws_lambda_permission" "allow_cloudwatch" {
  ...
  function_name = "${aws_lambda_function.es_cleanup[0].arn}"
...
resource "aws_cloudwatch_event_target" "es_cleanup_vpc" {
  ...
  target_id = "${var.prefix}lambda-es-cleanup"
  rule      = "${aws_cloudwatch_event_rule.schedule.name}"
  arn       = "${aws_lambda_function.es_cleanup_vpc[0].arn}"
}
...
resource "aws_lambda_permission" "allow_cloudwatch_vpc" {
  ...
  statement_id  = "AllowExecutionFromCloudWatch"
  action        = "lambda:InvokeFunction"
  function_name = "${aws_lambda_function.es_cleanup_vpc[0].arn}"
  principal     = "events.amazonaws.com"
  source_arn    = "${aws_cloudwatch_event_rule.schedule.arn}"
}
